...
export default async function ApartmentDetailPage({ params }: ApartmentDetailPageProps) {
  ...
  return (
    <div className="container mx-auto px-4 pt-36 md:pt-40 pb-12 space-y-10">
      <div className="space-y-3">
        <h1 className="text-4xl font-bold text-gray-900">{apartment.name}</h1>
        <p className="text-lg text-gray-600">
          {apartment.short_description || 'Artful stay designed for comfort and inspiration.'}
        </p>
        <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
          <span className="inline-flex items-center gap-1"><Users className="h-4 w-4" /> {apartment.max_guests} guests</span>
          <span className="inline-flex items-center gap-1"><Bed className="h-4 w-4" /> {apartment.bedrooms} bedrooms</span>
          <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> Venice, Italy</span>
        </div>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <section className="lg:col-span-2 space-y-4">
          <h2 className="text-2xl font-semibold text-gray-900">About this apartment</h2>
          <p className="text-gray-700 leading-7 whitespace-pre-line">{apartment.description}</p>
        </section>
